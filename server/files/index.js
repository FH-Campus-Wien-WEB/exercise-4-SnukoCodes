  ...
  fetch(`/search?query=${encodeURIComponent(query)}`)
    ...
    .then(results => {
      ...
      results.forEach(movie => {
        ...
        entry.append(new ElementBuilder("span").text(`${movie.Title} (${movie.Year})`));
        entry.append(new ButtonBuilder("Add").onclick(() => addMovie(movie.imdbID)));
...
window.onload = function () {
  ...
  function renderUserGreeting() {
    ...
    if (currentSession) {
      ...
      const datePart = loginDate.toLocaleDateString('de-DE', { day: 'numeric', month: 'long', year: 'numeric' });
      const timePart = loginDate.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' });
      greetingElement.textContent = `Hi ${currentSession.firstName} ${currentSession.lastName}, du hast dich am ${datePart} um ${timePart} angemeldet.`;
    } else {
